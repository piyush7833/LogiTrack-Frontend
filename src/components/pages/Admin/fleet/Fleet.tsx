...
const Fleet = () => {
  ...
  const { vehicles } = useSelector(
    (state: { vehicle: { vehicles: VehicleType[] } }) => state.vehicle
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      await getVehicle();
      await getDriver();
      const res = await getAllVehicleAnalytics();
      console.log(res);
      setVehicleAnalytics(res?.data.data.allVehicleAnalytics);
    };
    fetchData();
  }, []);
    ...
    <div className="min-h-screen p-2">
      ...
      <div className="w-full flex gap-2">
        <div className="flex w-2/3 flex-wrap gap-4 justify-center ">
          {vehicles.length > 0 &&
            vehicles.map((item: VehicleType, index: any) => (
              <CommonCard
                key={index}
                title={item?.model}
                desc={item?.numberPlate}
                vehicleType={item?.type}
                driver={item?.driver?.name || "No driver"}
                onDelete={() => {
                  deleteVehicle(item?._id as string);
                }}
                onEdit={() => {
                  setIsEditOpen(item._id);
                }}
                img={imgMaps[item.type as keyof typeof imgMaps]}
                onClick={() => {
                  router.push(
                    `/admin/fleet/${item._id}?model=${item.model}&numberPlate=${item.numberPlate}`
                  );
                }}
              />
            ))}
        ...
        <div className="w-1/3">
         {vehicleAnalytics && <LineBarGraph graphData={vehicleAnalytics} valueKey="count" xValueKey="type" XLabel="Vehicle Type" YLabel="No. of Bookings" isArea isLine title="Booking count by vehicle type" />}
        </div>
      </div>
      ...
      >
        <form onSubmit={(e) => handleUpdateVehicle(e)}>
          <div className="mb-4">
            ...
            >
              {["select type", "truck", "mini truck", "big truck"].map(
                (option) => (
